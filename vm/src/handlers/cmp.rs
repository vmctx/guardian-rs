...
pub unsafe fn cmp(vm: &mut Machine, op_size: OpSize) {
    match op_size {
        OpSize::Qword => {
            ...
            calculate_rflags!(vm, i64, op1, op2, result, sub, OF, SF, ZF, PF, CF);
        },
        OpSize::Dword => {
            let (op2, op1) = (vm.stack_pop::<u32>(), vm.stack_pop::<u32>());
            let result = op1.wrapping_sub(op2);
            calculate_rflags!(vm, i32, op1, op2, result, sub, OF, SF, ZF, PF, CF);
        },
        OpSize::Word => {
            let (op2, op1) = (vm.stack_pop::<u16>(), vm.stack_pop::<u16>());
            let result = op1.wrapping_sub(op2);
            calculate_rflags!(vm, i16, op1, op2, result, sub, OF, SF, ZF, PF, CF);
        },
        OpSize::Byte => {
            let (op2, op1) = (vm.stack_pop::<u16>() as u8, vm.stack_pop::<u16>() as u8);
            let result = op1.wrapping_sub(op2);
            calculate_rflags!(vm, i8, op1, op2, result, sub, OF, SF, ZF, PF, CF);
        },
    }
}